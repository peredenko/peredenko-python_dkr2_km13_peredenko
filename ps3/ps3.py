...
WORDLIST_FILENAME = "c:\КПИ\ps3\words.txt"
...
def get_word_score(word, n):
    ...
    max1=max(1, (7*len(word)-3*(n-len(word))))
    return sum1*max1
...
def is_valid_word(word, hand, word_list):
    ...
    word=word.lower()                     #не повністю реалізована
    hand2 = hand.copy()
...
def play_hand(hand, word_list):
    ...
    * The hand finishes when there are no more unused letters.
    ...
    while True:
        ...
        if word == '!!':
            break
        if is_valid_word(word, hand2, word_list):
            total_score += get_word_score(word, calculate_handlen(hand2))
...
def substitute_hand(hand, letter):
    ...
    for i in handlist:
        ...
        if i not in VOWELS:
            consonants = consonants.replace(i, '')  
